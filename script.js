      ...
      function loadData() {
        const saved = localStorage.getItem("angelina");
        if (saved) tasks = JSON.parse(saved);
        else
          tasks = [
            {
              id: 1,
              title: "Evaluate the addition awherend deletion of user IDs",
              status: "pending",
              priority: "later",
              completed: false,
            },
      ...
      function updateGreeting() {
        ...
        document.getElementById("greeting").textContent = `${greet}, Angelina!`;
      }

      function saveData() {
        localStorage.setItem("angelina", JSON.stringify(tasks));
      }
